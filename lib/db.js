...
function unwrap(data) {
  ...
  if (val?.value !== undefined) {
    try { return typeof val.value === 'string' ? JSON.parse(val.value) : val.value; } catch { return val.value; }
  }
  return val;
}
...
function toArray(val) {
  ...
  if (val?.value) { try { return JSON.parse(val.value); } catch { return []; } }
  return [];
}

function toObject(val) {
  if (!val) return {};
  if (typeof val === 'object' && !Array.isArray(val) && !val?.value) return val;
  if (typeof val === 'string') { try { return JSON.parse(val); } catch { return {}; } }
  if (val?.value) { try { return JSON.parse(val.value); } catch { return {}; } }
  return {};
}
...
function toString(val) {
  ...
  if (val?.value) return typeof val.value === 'string' ? val.value : JSON.stringify(val.value);
  return String(val);
}

export async function getUsers() { return toObject(await kget('nt:users')); }
...
export async function getRequests() { return toArray(await kget('nt:requests')); }
...
export async function getRawFacts(userName) {
  return toArray(await kget(`nt:raw:${userName.toLowerCase()}`));
}
...
export async function getProfile(userName) {
  return toString(await kget(`nt:profile:${userName.toLowerCase()}`));
}
export async function setProfile(userName, summary) {
  await kset(`nt:profile:${userName.toLowerCase()}`, summary);
}

export async function getGroupLore() { return toString(await kget('nt:lore')); }
...
export async function getStory() { return toString(await kget('nt:story')); }
...
export async function getUserMessages(userName) {
  return toArray(await kget(`nt:msgs:${userName.toLowerCase()}`));
}
...
export async function getSessions(userName) {
  return toArray(await kget(`nt:sessions:${userName.toLowerCase()}`));
}
...
export async function getSessionMessages(userName, sessionId) {
  return toArray(await kget(`nt:sess-msgs:${userName.toLowerCase()}:${sessionId}`));
}
...
export async function saveOTP(email, data) {
  await kset(`nt:otp:${email.toLowerCase()}`, data);
}
export async function getOTP(email) {
  return (await kget(`nt:otp:${email.toLowerCase()}`)) || null;
}
export async function deleteOTP(email) {
  await kset(`nt:otp:${email.toLowerCase()}`, null);
}
